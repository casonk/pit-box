...
# --index serves the toolbar-injected HTML generated by scripts/inject_toolbar.py.
ExecStart=/usr/bin/ttyd --writable --interface 127.0.0.1 --port ${WEBTERM_PORT} --ping-interval 30 --index /etc/pit-box/webterm/index.html tmux new-session -A -s pit-box
Restart=on-failure
RestartSec=5
